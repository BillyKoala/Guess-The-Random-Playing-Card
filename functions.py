...
def mouseClick(event):
    ...
    try:
        ...
        # Remove the selected card from the window. *** Not working yet ***
        for card in globals.card_deck:
...
def get_player_hint(selected_card):
    ...
    try:
        ...
        if player_card_suit_index == random_card_suit_index and player_card_number_index < random_card_number_index:
            ...
            if globals.attempt_count == 1: # Did the player get it first time?
        ...
        # The last attempt was incorrect. Show the selected card and offer a hint.
